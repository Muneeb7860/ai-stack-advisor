...
| 18 | `18-access-control-four-planes.md` | Access control as four planes (end-user, service-to-service, human-operator, workload-to-resource): coarse/fine-grained authz split, token revocation, SVID/attestation, secret zero, PIM/break-glass, infra-vs-data access, audit-event production | Partial — `pickMesh()`/a trade-off card cover plane 2's SPIFFE/SPIRE identity; PIM, break-glass, access reviews, data-access auditing, and same-cloud workload identity have no representation |
