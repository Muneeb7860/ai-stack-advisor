...
children.push(bulletBold('Harness Readiness — evidence upload — ', 'an optional "have a file that proves this?" attachment on each of the five audit questions. The browser reads one user-picked file (an <code>AGENTS.md</code>, a CI config, a permissions file, a failure log), checks it against the level the user selected for themselves, and either confirms it or flags the mismatch with a specific reason — it never overrides the user\'s own answer, and never blocks. This addresses the one real weakness of a pure self-audit (nothing stops a team scoring themselves top marks on every question) without abandoning the low-friction questionnaire: skipping every upload leaves the flow exactly as it was. Client-side only — the file is read in the browser and discarded, never transmitted.'));
children.push(bulletBold('LLM Observability — ', 'a new recommendation category (Langfuse / Braintrust) covering tracing and evaluation of LLM calls. The product recommended AI stacks but had nothing to say about observing the model calls it was recommending — general infrastructure observability (Datadog/Grafana) was already covered; LLM-specific observability was a genuine gap in a product whose whole subject is AI architecture.'));
children.push(bulletBold('Answer-first results — ', 'the report opens with the recommendation itself (headline pick plus a cloud/database/compute line-up) instead of the detected-signal count and signal chips that previously filled the first screen, which restated the user\'s own input back at them. The 19 report sections and 24 stack cards now start collapsed: a card shows its category and the chosen technology, with the reasoning, cited basis, alternatives and refine/ask controls one click away. Nothing was removed — exports and share links still carry the full analysis.'));
children.push(bulletBold('"Needs your attention" — ', 'a short ranked list of the picks genuinely worth a second look before committing, surfacing the combination that matters most: a choice that is both expensive to reverse and only weakly supported by the requirement text. Both inputs already existed in the engine; nothing surfaced their intersection before, and it is the single most useful thing this product can tell someone at the point of decision.'));
children.push(bulletBold('Architecture canvas as the default view — ', 'the node canvas showing how the recommended components connect is now what a user lands on, with the card view one click away. Made possible by a single-column canvas layout for phones — the six-tier layout previously shrank to a quarter scale on a narrow screen and was unreadable, so defaulting to it would have been worse than not.'));
children.push(bulletBold('Harness Readiness — history and feedback — ', 'audits are saved locally and a repeat audit shows what moved per component, which is what makes the "fix your weakest area, then re-audit" loop the rubric describes actually followable. The results screen also asks whether the audit was useful — the first path by which anything from this feature area reaches us rather than staying in the user\'s browser.'));
children.push(bulletBold('Deployable without source edits — ', 'the frontend resolves its API address from configuration rather than a hardcoded local one, and the container image runs its own database migrations and binds the port its host assigns. Without these, a hosted copy would have silently discarded every piece of user feedback and started against an empty database. No infrastructure has been provisioned — see the GCP deployment plan.'));
children.push(bulletBold('Keyboard-first navigation — ', 'a command palette on Cmd/Ctrl-K that searches actions, report sections and individual recommendations, plus a shortcut to jump straight to the requirement box. Matching is forgiving — typing "expsvg" finds "Export diagram as SVG" — so the palette is usable from memory rather than requiring exact wording. This was the largest gap between the product and the tools it is measured against.'));
children.push(bulletBold('Visual density — ', 'blocks sitting inside a card no longer draw their own border, letting a subtle change of surface do the work instead. Fixing this first required giving the light theme its own distinct surface tone, which had been identical to the card colour.'));
children.push(bulletBold('A landing page with its own voice — ', 'the marketing page now carries a display typeface, a signature gradient on its calls to action and subtle motion, while keeping the product palette so it still looks like the tool it advertises. Deliberately not applied to the application itself, where that same decoration would read as generic.'));
children.push(bulletBold('Readable in both themes — ', 'the colours carrying "recommended", "adjusted" and "caution" were fixed to the dark theme\'s values, so in the light theme the guardrails list and the marker showing which vendor was recommended were washed out to the point of being unreadable. Both now resolve per theme, with separate tokens for text, since a colour that reads well as a highlight on a dark background does not read as text on a white one.'));
children.push(bulletBold('Evidence that disagrees in your favour — ', 'attaching a file that showed MORE than the level a user selected was reported as confirming that level. The audit then pointed them at the lowest-scoring area as the place to start, which could be something they demonstrably already had. The check now says what the file actually shows, and still never changes the answer for them.'));
children.push(bulletBold('Usable from a coding agent — ', 'the MCP server has been shipped and tested for some time but appeared nowhere in the product, so only someone reading the backend source would know it existed. There is now an install panel, a sidebar entry and a landing-page section, available before running an analysis. This extends reach rather than capability: the same engine, reachable from where the work actually happens.'));
children.push(bulletBold('Start from what you already run — ', 'a user with an existing system can upload a dependency file (package.json, requirements.txt, go.mod, Gemfile, pom.xml or docker-compose.yml) instead of writing out their stack. Recognised technologies are shown for confirmation before anything is analysed, and anything the product cannot actually reason about is left out rather than displayed, so the list never implies more understanding than there is.'));
children.push(bulletBold('Where AI-written code should run — ', 'a new category covering isolation for stacks that execute code a model generates. This is a different decision from where your own services run, which the existing categories already cover: here the code being executed is the thing you are protecting against. The comparison also records that these vendors\' published start-up times are measured one request at a time, while an AI agent creates them in bursts — under which the fastest-advertised option performs worst.'));
children.push(bulletBold('Consistency across categories — ', 'an audit found several recently-added categories were not connected to everything that reads a category, most visibly the feature that lets a user say they would have chosen differently: for three categories it offered an empty box instead of the real alternatives. No recommendation was ever wrong as a result. The checks that should have caught this were written in a way that could not, and have been rewritten to derive what they expect from the product itself.'));
children.push(bulletBold('Details you state are now used — ', 'naming a concurrency figure, a team size, or a delivery deadline changes the recommendation. Previously all three were read from the requirement and then ignored, so a carefully specified brief produced the same answer as a vague one. That is worse than not reading them: the interface acknowledged the detail and the recommendation did not reflect it.'));
children.push(bulletBold('Asking "what if" without rewriting the brief — ', 'four controls above the results let a reader change the assumed team size, scale, compliance posture or company stage and see which recommendations move. The middle position on each is always what they actually wrote, so the controls explore alternatives rather than correcting them, and the product reports plainly when a change makes no difference.'));
children.push(bulletBold('A downloadable remediation pack — ', 'after a harness audit, a one-click download of a starter instruction file, permissions policy and failure log tailored to the gaps the audit found. Reviewed before release: the tailoring was reading score names that did not exist, so every user received the same worst-case advice regardless of their answers — including anyone who scored full marks.'));
