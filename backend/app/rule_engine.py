...
EXCLUSION_TERMS = {
    ...
    # Bare "ai" added after a real report: "strictly no cloud and ai" excluded the cloud
    # half and silently kept every AI card. Safe despite its length — find_exclusions()
    # anchors every term with \b...\b, so it cannot fire inside "email"/"available".
    "llm": ["llm", "llms", "large language model", "gpt", "claude", "openai", "genai", "generative ai", "ai", "artificial intelligence"],
...
def pick_iam(s):
    if s["onPrem"]:
        primary = {"id": "selfhosted", "v": "Self-hosted open-source IdP (Keycloak or FreeIPA)", "why": "Okta, Entra ID, OneLogin, JumpCloud, Clerk, WorkOS and Saviynt are hosted services and cannot run inside an air-gapped boundary at all. Ping, Oracle, CyberArk and SailPoint do ship self-managed product lines and stay in the comparison below, but each needs its specific edition confirmed rather than assumed. A self-hosted, open-source IdP inside your network boundary is the realistic default.", "conf": "high"}
    ...
    # An air-gapped requirement excludes hosted-only products from the comparison, not just
    # from the pick — same defect class as the domain floors (AGENTS.md).
    result["alternatives"] = [v for v in IAM_VENDORS if v["deploy"] != "saas"] if s["onPrem"] else IAM_VENDORS
...
COMPUTE_VENDORS = [
    ...
    {"id": "coolify", "name": "Coolify", "cat": "Self-hosted PaaS (Docker Compose-based)", "bestFor": "Teams wanting a Heroku/Vercel-style git-push deploy experience (managed TLS, one-click databases, preview deployments) entirely on their own servers", "strength": "Free and open source with no feature gating on the self-hosted version; connects any server from any provider (Hetzner, AWS, bare metal); removes most of the daily-ops overhead of running raw Docker Compose or Kubernetes by hand", "drawback": "You still own the underlying server(s) — no managed infra, no SLA; smaller plugin/integration ecosystem than the SaaS PaaS options above; self-hosted-only support is community-based unless you pay for Coolify Cloud", "pricing": "Self-hosted: free forever, open source, unlimited servers · Coolify Cloud (hosted control plane only): $5/mo (2 servers) + $3/mo per additional server"},
    {"id": "dokploy", "name": "Dokploy", "cat": "Self-hosted PaaS (Docker Compose-based)", "bestFor": "Teams wanting a Coolify-equivalent self-hosted PaaS, evaluated as a direct alternative before settling on one", "strength": "Open source and self-hostable for free; positions itself explicitly around \"no vendor lock-in\" and full control to modify/customize the platform itself, not just the apps deployed on it", "drawback": "Younger and smaller community than Coolify; managed cloud-hosting pricing model is per-server rather than a flat platform fee, which scales less predictably for a fleet of many small servers", "pricing": "Self-hosted: free, open source · Managed cloud hosting: Hobby $4.50/mo/server · Startup $15/mo (3 servers) · Enterprise custom"},
]


def pick_compute_platform(s, compute):
    if s["coolifyMentioned"]:
        return {"v": "Coolify", "why": "Explicit Coolify mention detected — matching existing familiarity over evaluating a new self-hosted PaaS. Free and open source to self-host; the managed Coolify Cloud offering just hosts the control plane, your apps still deploy to servers you connect yourself.", "primaryId": "coolify", "conf": "high"}
    if s["dokployMentioned"]:
        return {"v": "Dokploy", "why": "Explicit Dokploy mention detected — matching existing familiarity. Open source and free to self-host; the paid cloud tiers are for Dokploy's own managed hosting of the control plane, not a requirement to use the tool.", "primaryId": "dokploy", "conf": "high"}
    if s["onPrem"]:
        return {"v": "Coolify or Dokploy (self-hosted PaaS, Docker Compose-based) on top of your own infrastructure — or skip a PaaS layer entirely and run services directly on the self-managed Kubernetes from the Compute Model card above", "why": "The managed PaaS/FaaS platforms in this comparison (Vercel, Lambda, Cloud Functions, ...) are public-cloud services and aren't available air-gapped, so this card doesn't just say \"N/A\": Coolify and Dokploy are free, open-source, self-hostable PaaS layers (Heroku/Vercel-style git-push deploys, managed TLS, one-click databases) that run entirely inside your own network boundary — a meaningfully easier day-to-day experience than raw Kubernetes manifests if your team wants that convenience without a public-cloud dependency.", "primaryId": "coolify", "conf": "medium"}
    v = compute["v"]
...
def apply_exclusions(rec: dict, s: dict) -> dict:
    ...
    if ex.get("llm"):
        ...
        # Guardrails exist to constrain an LLM's output; with LLMs excluded there is nothing for
        # them to guard. This was the one AI card that survived an explicit "no AI" requirement.
        _suppress("guardrails_vendor")
